...
st.sidebar.write("Choose one or two layers to visualize and compare on the map.")

# Option to choose single or double layer comparison
comparison_mode = st.sidebar.radio("Comparison Mode", ["Single Layer", "Compare Two Layers"])

# Get layer selection based on the comparison mode
if comparison_mode == "Single Layer":
    selected_layers = st.sidebar.multiselect("Choose one layer:", gpkg_files.keys(), max_selections=1)
else:
    selected_layers = st.sidebar.multiselect("Choose two layers:", gpkg_files.keys(), max_selections=2)
...
if comparison_mode == "Single Layer" and selected_gdfs:
    left_col, right_col = st.columns([3, 2])
    with left_col:
        # Display single layer map
        ...
        layers = [
            ...
            for color in color_ranges[selected_layers[0]]
        ]
    ...
    with right_col:
        # Display interactive graphs and charts for the selected layer
        for gdf, layer in zip(selected_gdfs, selected_layers):
            ...
            st.write(f"{layer} Data Summary")
            st.dataframe(gdf[['st_nm', 'value_column']].sort_values(by="value_column", ascending=False))

elif comparison_mode == "Compare Two Layers" and len(selected_gdfs) == 2:
    left_col, right_col = st.columns(2)
    
    for i, (gdf, layer) in enumerate(zip(selected_gdfs, selected_layers)):
        with (left_col if i == 0 else right_col):
            combined_gdf = gdf.to_crs(epsg=4326)
            layers = [
                pdk.Layer(
                    "GeoJsonLayer",
                    data=combined_gdf[combined_gdf['color'] == color].__geo_interface__,
...
# Option to download map as PNG
download_button = st.sidebar.button("Download Map as PNG")
if download_button:
    buffer = BytesIO()
    fig.savefig(buffer, format="png")
    st.sidebar.download_button("Download Map as PNG", buffer.getvalue(), "map.png")
